...
import burgerImage from "../assets/burgermenu.jpg"
...
export default function BurgerMenu() {
  ...
  const [addOns, setAddOns] = useState([
    { name: "EXTRA CHEESE", price: 20, quantity: 0 },
    { name: "BACON STRIPS", price: 35, quantity: 0 },
    { name: "GUACAMOLE", price: 30, quantity: 0 },
    { name: "ONION RINGS", price: 15, quantity: 0 },
    { name: "BBQ SAUCE", price: 10, quantity: 0 },
  ])
  ...
  const calculateTotal = () => {
    ...
    return selectedBurger ? selectedBurger.price + addOnsTotal : 0
  }
      ...
      setBurgers(prev => 
        prev.map(burger => 
          burger._id === selectedBurger._id
            ? { ...burger, quantity: burger.quantity - 1 }
            : burger
        )
      )
    } catch (error) {
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">BURGERS</h1>
            <p className="text-xl text-gray-200">Juicy patties with gourmet toppings</p>
          </div>
        </div>
        <img src={burgerImage} alt="Burger Dish" className="object-cover w-full h-full" />
      </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Asia</span> <span className="text-[#122348]">Burgers</span>
          </h2>
          <p className="text-gray-600 mt-1">Handcrafted burgers with premium ingredients</p>
        </div>
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative hidden md:block md:w-2/5 h-auto bg-[#122348]">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-3xl font-bold text-white text-center px-4">{selectedBurger?.name} Burger</h2>
                </div>
              </div>
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="md:hidden mb-3">
                  <h2 className="text-xl font-bold text-[#122348]">
                    {selectedBurger?.name} <span className="text-[#ff3131]">Burger</span>
                  </h2>
                </div>
                
                <p className="text-gray-600 text-sm italic mb-3">{selectedBurger?.description}</p>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                  ...
                  <Button
                    ...
                    onClick={addToCart}
                    disabled={!selectedBurger || !tableNumber}
                  >
                    ADD TO CART
                  </Button>
                </div>
