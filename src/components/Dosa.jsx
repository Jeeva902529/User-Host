...
import dosaImage from "../assets/dosa-menu.jpg" // New image path

export default function DosaMenu() {
  const [dosaVarieties, setDosaVarieties] = useState([])
  const [selectedDosa, setSelectedDosa] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "EXTRA SAMBAR", price: 15, quantity: 0 },
    { name: "COCONUT CHUTNEY", price: 10, quantity: 0 },
    { name: "ONION CHUTNEY", price: 10, quantity: 0 },
    { name: "CHEESE MELT", price: 25, quantity: 0 },
    { name: "BUTTER TOPPING", price: 20, quantity: 0 },
  ])
  ...
  const calculateTotal = () => {
    ...
    return selectedDosa ? selectedDosa.price + addOnsTotal : 0
  }

  const addToCart = async () => {
    if (!token || !tableNumber || !selectedDosa) {
    ...
    try {
      ...
      setDosaVarieties(prev =>
        prev.map(dosa =>
          dosa._id === selectedDosa._id
            ? { ...dosa, quantity: dosa.quantity - 1 }
            : dosa
        )
      )
    } catch (error) {
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">DOSA</h1>
            <p className="text-xl text-gray-200">Crispy South Indian delicacy</p>
          </div>
        </div>
        <img src={dosaImage} alt="Dosa Dish" className="object-cover w-full h-full" />
      </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Asia</span> <span className="text-[#122348]">Dosa</span>
          </h2>
          <p className="text-gray-600 mt-1">Authentic South Indian crispy crepes</p>
        </div>

        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {dosaVarieties.map((item, index) => (
              <div
                key={index}
          ...
          <DialogContent className="bg-white max-w-md md:max-w-3xl rounded-xl shadow-lg p-0 overflow-hidden">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative hidden md:block md:w-2/5 h-auto bg-[#122348]">
                <img
                  src={dosaImage}
                  alt={selectedDosa?.name}
                  className="w-full h-full object-cover opacity-80"
                />
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-3xl font-bold text-white text-center px-4">{selectedDosa?.name} Dosa</h2>
                </div>
              </div>
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="md:hidden mb-3">
                  <h2 className="text-xl font-bold text-[#122348]">
                    {selectedDosa?.name} <span className="text-[#ff3131]">Dosa</span>
                  </h2>
                </div>
                
                <p className="text-gray-600 text-sm italic mb-3">{selectedDosa?.description}</p>
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., extra crispy, less spicy)"
                    value={customNotes}
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                  ...
                  <Button
                    ...
                    onClick={addToCart}
                    disabled={!selectedDosa || !tableNumber}
                  >
                    ADD TO CART
                  </Button>
                </div>
