...
import browniesImage from "../assets/browniesmenu.jpg" // New image path

export default function BrowniesMenu() {
  const [browniesVarieties, setBrowniesVarieties] = useState([])
  const [selectedBrownie, setSelectedBrownie] = useState(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [addOns, setAddOns] = useState([
    { name: "EXTRA CHOCOLATE CHIPS", price: 20, quantity: 0 },
    { name: "CARAMEL DRIZZLE", price: 15, quantity: 0 },
    { name: "SPRINKLES", price: 10, quantity: 0 },
    { name: "NUTS ADDITION", price: 25, quantity: 0 },
    { name: "FROSTING LAYER", price: 30, quantity: 0 },
  ])
  ...
  const calculateTotal = () => {
    ...
    return selectedBrownie ? selectedBrownie.price + addOnsTotal : 0
  }

  const addToCart = async () => {
    if (!token || !tableNumber || !selectedBrownie) {
      console.error("Missing required fields")
      return
    }

    const orderData = {
      foodName: selectedBrownie.name,
      basePrice: selectedBrownie.price,
      ...
      setBrowniesVarieties(prev =>
        prev.map(brownie =>
          brownie._id === selectedBrownie._id
            ? { ...brownie, quantity: brownie.quantity - 1 }
            : brownie
        )
      )
    } catch (error) {
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">BROWNIES</h1>
            <p className="text-xl text-gray-200">Decadent chocolate treats with gourmet twists</p>
          </div>
        </div>
        <img src={browniesImage} alt="Brownie Dish" className="object-cover w-full h-full" />
      </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Asia</span> <span className="text-[#122348]">Brownies</span>
          </h2>
          <p className="text-gray-600 mt-1">Rich chocolate creations with global flavors</p>
        </div>

        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {browniesVarieties.map((item, index) => (
              <div
                key={index}
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
                        item.type === "chocolate" ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800"
                      }`}>
                        {item.type === "chocolate" ? "Chocolate" : "Nutty"}
                      </span>
                    </div>
                  </div>
          ...
          <DialogContent className="bg-white max-w-md md:max-w-3xl rounded-xl shadow-lg p-0 overflow-hidden">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative hidden md:block md:w-2/5 h-auto bg-[#122348]">
                <img
                  src={browniesImage}
                  alt={selectedBrownie?.name}
                  className="w-full h-full object-cover opacity-80"
                />
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-3xl font-bold text-white text-center px-4">{selectedBrownie?.name} Brownie</h2>
                </div>
              </div>
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="md:hidden mb-3">
                  <h2 className="text-xl font-bold text-[#122348]">
                    {selectedBrownie?.name} <span className="text-[#ff3131]">Brownie</span>
                  </h2>
                </div>
                
                <p className="text-gray-600 text-sm italic mb-3">{selectedBrownie?.description}</p>
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., extra fudge, no nuts)"
                    value={customNotes}
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                  ...
                  <Button
                    ...
                    onClick={addToCart}
                    disabled={!selectedBrownie || !tableNumber}
                  >
                    ADD TO CART
                  </Button>
                </div>
