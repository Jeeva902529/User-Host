...
export default function GulabJamunMenu() {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://back-end-res-6emf.onrender.com/api/foods/gulabjamun")
      .then((response) => setGulabJamuns(response.data))
      .catch((error) => console.error("Error fetching gulab jamun data:", error))
  }, [])
  ...
  const addToCart = async () => {
    ...
    try {
      ...
      // Update inventory
      await axios.patch(
        `https://back-end-res-6emf.onrender.com/api/foods/${selectedGulabJamun._id}/decrease-quantity`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      )
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            ...
            <p className="text-xl text-gray-200">Traditional Indian sweets soaked in rose syrup</p>
          </div>
        </div>
        <img src={gulabJamunImage} alt="Gulab Jamun" className="object-cover w-full h-full" />
      </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Asia</span> <span className="text-[#122348]">Gulab Jamun</span>
          </h2>
          <p className="text-gray-600 mt-1">Soft, syrupy delights with aromatic spices</p>
        </div>
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative hidden md:block md:w-2/5 h-auto bg-[#122348]">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-3xl font-bold text-white text-center px-4">{selectedGulabJamun?.name} Gulab Jamun</h2>
                </div>
              </div>
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="md:hidden mb-3">
                  <h2 className="text-xl font-bold text-[#122348]">
                    {selectedGulabJamun?.name} <span className="text-[#ff3131]">Gulab Jamun</span>
                  </h2>
                </div>
                
                <p className="text-gray-600 text-sm italic mb-3">{selectedGulabJamun?.description}</p>
