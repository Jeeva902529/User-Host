...
import pieImage from "../assets/piemenu.jpg" // New image path
...
export default function PieMenu() {
  ...
  const [addOns, setAddOns] = useState([
    ...
    { name: "ICE CREAM SCOOP", price: 30, quantity: 0 },
    { name: "EXTRA FILLING", price: 25, quantity: 0 },
    { name: "CRUMBLE TOPPING", price: 15, quantity: 0 },
    { name: "DRIZZLE SAUCE", price: 10, quantity: 0 },
  ])
  ...
  const calculateTotal = () => {
    ...
    return selectedPie ? selectedPie.price + addOnsTotal : 0
  }

  const addToCart = async () => {
    if (!token || !tableNumber || !selectedPie) {
      console.error("Missing required fields")
      return
    }

    const orderData = {
      foodName: selectedPie.name,
      basePrice: selectedPie.price,
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">PIES</h1>
            <p className="text-xl text-gray-200">Classic baked desserts with modern twists</p>
          </div>
        </div>
        <img src={pieImage} alt="Pie Dish" className="object-cover w-full h-full" />
      </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Asia</span> <span className="text-[#122348]">Pies</span>
          </h2>
          <p className="text-gray-600 mt-1">Authentic baked pies with global flavors</p>
        </div>
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
                        item.type === "sweet" ? "bg-pink-100 text-pink-800" : "bg-blue-100 text-blue-800"
                      }`}>
                        {item.type === "sweet" ? "Sweet" : "Savory"}
                      </span>
                    </div>
                  </div>
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative hidden md:block md:w-2/5 h-auto bg-[#122348]">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-3xl font-bold text-white text-center px-4">{selectedPie?.name} Pie</h2>
                </div>
              </div>
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="md:hidden mb-3">
                  <h2 className="text-xl font-bold text-[#122348]">
                    {selectedPie?.name} <span className="text-[#ff3131]">Pie</span>
                  </h2>
                </div>
                
                <p className="text-gray-600 text-sm italic mb-3">{selectedPie?.description}</p>
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., extra filling, no whipped cream)"
                    value={customNotes}
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                  ...
                  <Button
                    ...
                    onClick={addToCart}
                    disabled={!selectedPie || !tableNumber}
                  >
                    ADD TO CART
                  </Button>
                </div>
