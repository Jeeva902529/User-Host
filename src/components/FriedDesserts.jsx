...
export default function FriedDessertsMenu() {
  ...
  const [addOns, setAddOns] = useState([
    { name: "CHOCOLATE SAUCE", price: 20, quantity: 0 },
  ...
  const calculateTotal = () => {
    ...
    return selectedDessert ? selectedDessert.price + addOnsTotal : 0
  }

  const addToCart = async () => {
    if (!token || !tableNumber || !selectedDessert) {
      console.error("Missing required fields")
      return
    }

    const orderData = {
      foodName: selectedDessert.name,
      basePrice: selectedDessert.price,
    ...
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      <div className="relative h-[40vh] md:h-screen md:w-1/2 lg:w-3/5">
        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-black/20 flex items-end p-8">
          <div>
            <h1 className="text-5xl font-bold text-white mb-2">FRIED DESSERTS</h1>
            <p className="text-xl text-gray-200">Crispy sweet delights with global flavors</p>
          </div>
        </div>
        <img src={friedDessertsImage} alt="Fried Desserts" className="object-cover w-full h-full" />
      </div>
      ...
      <div className="flex flex-1 flex-col bg-gray-50 md:h-screen md:overflow-hidden">
        <div className="sticky top-0 bg-gray-50 p-6 pb-4 z-10 shadow-sm">
          ...
          <h2 className="text-3xl font-bold">
            <span className="text-[#ff3131]">Flavors of Asia</span> <span className="text-[#122348]">Fried Desserts</span>
          </h2>
          <p className="text-gray-600 mt-1">Crispy, sweet, and irresistibly delicious</p>
        </div>

        <div className="flex-1 overflow-y-auto p-6 pt-2">
          <div className="grid gap-4">
            {friedDessertsVarieties.map((item, index) => (
              <div
                key={index}
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
                        item.type === "gluten-free" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                      }`}>
                        {item.type === "gluten-free" ? "Gluten-Free" : "Regular"}
                      </span>
                    </div>
                  </div>
          ...
          <DialogContent className="bg-white max-w-md md:max-w-3xl rounded-xl shadow-lg p-0 overflow-hidden">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative hidden md:block md:w-2/5 h-auto bg-[#122348]">
                <img
                  src={friedDessertsImage}
                  alt={selectedDessert?.name}
                  className="w-full h-full object-cover opacity-80"
                />
                <div className="absolute inset-0 flex items-center justify-center">
                  <h2 className="text-3xl font-bold text-white text-center px-4">{selectedDessert?.name} Dessert</h2>
                </div>
              </div>
              ...
              <div className="p-4 md:p-5 w-full md:w-3/5">
                ...
                <div className="md:hidden mb-3">
                  <h2 className="text-xl font-bold text-[#122348]">
                    {selectedDessert?.name} <span className="text-[#ff3131]">Dessert</span>
                  </h2>
                </div>
                
                <p className="text-gray-600 text-sm italic mb-3">{selectedDessert?.description}</p>
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    placeholder="Any special requests? (e.g., extra syrup, no ice cream)"
                    value={customNotes}
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                  ...
                  <Button
                    ...
                    onClick={addToCart}
                    disabled={!selectedDessert || !tableNumber}
                  >
                    ADD TO CART
                  </Button>
                </div>
